...
class ServicioTest {
    ...
    @Test
    void soliictudTest() throws Exception {
        Servicio servicio = getServicioRandomSampleGenerator();
        Solicitud solicitudBack = getSolicitudRandomSampleGenerator();

        servicio.setSoliictud(solicitudBack);
        assertThat(servicio.getSoliictud()).isEqualTo(solicitudBack);

        servicio.soliictud(null);
        assertThat(servicio.getSoliictud()).isNull();
    }
}
