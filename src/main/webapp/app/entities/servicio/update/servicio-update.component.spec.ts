...
describe('Servicio Management Update Component', () => {
  ...
  describe('ngOnInit', () => {
    it('Should call solicitud query and add missing value', () => {
      const servicio: IServicio = { id: 456 };
      const solicitud: ISolicitud = { id: 30639 };
      servicio.solicitud = solicitud;

      const solicitudCollection: ISolicitud[] = [{ id: 6434 }];
      jest.spyOn(solicitudService, 'query').mockReturnValue(of(new HttpResponse({ body: solicitudCollection })));
      const expectedCollection: ISolicitud[] = [solicitud, ...solicitudCollection];
      ...
      expect(solicitudService.addSolicitudToCollectionIfMissing).toHaveBeenCalledWith(solicitudCollection, solicitud);
      expect(comp.solicitudsCollection).toEqual(expectedCollection);
    });

    it('Should update editForm', () => {
      const servicio: IServicio = { id: 456 };
      const solicitud: ISolicitud = { id: 19643 };
      servicio.solicitud = solicitud;

      activatedRoute.data = of({ servicio });
      comp.ngOnInit();

      expect(comp.solicitudsCollection).toContain(solicitud);
