...
public class ServicioResource {
     ...
     */
    @GetMapping("")
    public List<Servicio> getAllServicios(@RequestParam(required = false) String filter) {
        if ("solicitud-is-null".equals(filter)) {
            log.debug("REST request to get all Servicios where solicitud is null");
            return StreamSupport
                .stream(servicioRepository.findAll().spliterator(), false)
                .filter(servicio -> servicio.getSolicitud() == null)
                .toList();
        }
